...
extern "C" bool to_bool(Scope_Struct *scope_struct, void *ptr) {
...
extern "C" DT_list *zip(Scope_Struct *scope_struct, DT_list *list, ...) {
  ...
  if (new_list==nullptr)
  {
    LogErrorC(scope_struct->code_line, "Zipping a single list");
    return list;
  }
  ...
  for (int i=1; i<lists.size(); ++i) {
    if (lists[i]->size!=list_size) {
      LogErrorC(scope_struct->code_line, "Zipping lists of different size.");
      return list;
    }
  }
